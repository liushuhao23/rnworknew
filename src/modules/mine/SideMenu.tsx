...
const Index = forwardRef((props: any, ref) => {
            ...
            return (
              <View key={`${index}`}>
                {item.map((subItem: any, subIndex) => {
                  return (
                    <TouchableOpacity key={`${index}-${subIndex}`} style={styles.menuItem} onPress={onMenuItemPress(subItem)}>
  ...
  return (
    <Modal transparent={true} visible={visible} statusBarTranslucent={true} animationType="fade" onRequestClose={hide}>
      <TouchableOpacity  onPress={hide} activeOpacity={1} style={styles.root}>{renderContent()}</TouchableOpacity>
    </Modal>
  );
});
